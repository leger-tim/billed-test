...
export default class {
  ...
  // New function without counter

  handleShowTickets(e, bills, index) {
    const arrowIcon = $(`#arrow-icon${index}`);
    const statusBillsContainer = $(`#status-bills-container${index}`);

    if (arrowIcon.attr("data-open") === "true") {
      // Si la liste est déjà ouverte, la refermer
      arrowIcon.css({ transform: "rotate(90deg)" });
      statusBillsContainer.html("");
      arrowIcon.attr("data-open", "false");
    } else {
      // Si la liste est fermée, l'ouvrir
      arrowIcon.css({ transform: "rotate(0deg)" });
      statusBillsContainer.html(cards(filteredBills(bills, getStatus(index))));
      arrowIcon.attr("data-open", "true");
    }

    // Attacher les événements de clic après le rendu des éléments
    filteredBills(bills, getStatus(index)).forEach((bill) => {
      $(`#open-bill${bill.id}`).off("click"); // Enlever tout événement de clic existant
